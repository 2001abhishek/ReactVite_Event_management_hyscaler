            ...
            <div className="container px-5 py-8 md:py-16 mx-auto  ">
                ...
                <div className="flex flex-wrap -m-4 ">
                    {product
                        ...
                        .slice(0, 8)
                        .map((item, index) => {
                            {/* slice(0,8) use for to show only 8 products on homepage */ }
                            const { title, price, description, imageUrl1, id } = item;
                            return (
                                <div onClick={() => openInNewTab(`/productinfo/${id}`)} target="_blank" rel="noopener noreferrer" key={index}
                                    ...
                                    <div className="h-full border-2 hover:shadow-black-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                        ...
                                        <div className="p-5 border-t-2">
                                            ...
                                            <div className=" flex justify-center">
                                                <button onClick={(event) => addCart(event, item)} type="button" className="focus:outline-none text-white bg-violet-600 hover:bg-violet-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full  py-2">Add To Cart</button>
